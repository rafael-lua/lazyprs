...
const cli = defineCommand({
  ...
  args: {
    ...
    from: {
      type: "string",
      description:
        "Branch or sha to use in merge-base for commit range. Default is {base}",
    },
    draft: {
      type: "boolean",
  ...
  async run({ args }) {
    ...
    const from = args.from || "origin/main"
    ...
    /*
        - Create a branch with {branchName} from {from}
        - Cherry pick the selected commits
        - Push to remote
        - Open a PR for the created branch against {base} using provided {title}
    */
